...
void myKeyboard(unsigned char key, int x, int y)
{
	switch (key)
	{
	case 'p': cam.slide(0, 0, -0.1); break;
	case 'l': cam.slide(0, 0, 0.1); break;
	case 'q': cam.roll(-camSpeed);  break;
	case 'w': cam.pitch(camSpeed); break;
	case 'e': cam.roll(camSpeed); break;
	case 's': cam.pitch(-camSpeed); break;

	case 'h': cam.rotate(Vector3(1.0, 1.0, 1.0), 1.0);  break;

	case 'd': cam.yaw(camSpeed); break;
	case 'a': cam.yaw(-camSpeed); break;
...
void init() {
	...
	for (z = 0; z < 21; z++) {
		for (x = 0; x < 21; x++) {
			...
			ground_colors[z][x][0] = r + 0.2; // red value
			ground_colors[z][x][1] = g + 0.6; // green value
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Project 2 Rain/snow/hail falling");
	init();
